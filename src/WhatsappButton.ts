...
export class WhatsappButton extends Button {
  ...
  loadingDialog() {
    if (this.shadowRoot) {
      ...
      dialog.renderer = (root: any) => {
        ...
        <p style="font-size:9px;display:${this._showButtonInDialog ? '' : 'none'
          }">If the chat doesn't open automatically, press the button below.</p>
        <a href="${this.link}"><vaadin-button style="display:${this._showButtonInDialog ? '' : 'none'
  ...
  _removeIcon() {
    ...
    this.shadowRoot
      ...
      .getElementsByTagName('slot')[0].children[0].remove();
  };
  ...
  protected update(_changedProperties: PropertyValueMap<any> | Map<PropertyKey, unknown>): void {
    if (_changedProperties.has("text") || _changedProperties.has("invitecode") || _changedProperties.has("phone") || _changedProperties.has("dialcode")) {
      this._updateLinkInfo();
    }
    // @ts-ignore
    if (this.shadowRoot.getElementById('button') && _changedProperties.has("leading")) {
